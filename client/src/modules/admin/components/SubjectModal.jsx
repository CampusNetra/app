...
const SubjectModal = ({ isOpen, onClose, onSubmit, creating, subject = null }) => {
      ...
      <div className="w-full max-w-md bg-white rounded-[2rem] shadow-[0_20px_70px_rgba(0,0,0,0.15)] overflow-hidden animate-in fade-in zoom-in-95 duration-300 border border-slate-100">
        <div className="px-10 py-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
          <div>
            ...
            <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest leading-relaxed">
              {subject ? 'Modify subject details' : 'Define a new subject'}
            </p>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8">
          <div className="space-y-3">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Subject Name</label>
            <input
              type="text"
              required
              autoFocus
              ...
              className="w-full px-6 py-5 rounded-2xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none text-base font-bold transition-all placeholder:text-slate-300 shadow-inner"
