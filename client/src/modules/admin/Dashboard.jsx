...
const Dashboard = () => {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw', overflow: 'hidden', background: '#fcfdfe' }}>
      <DashboardSidebar collapsed={collapsed} setCollapsed={setCollapsed} user={user} />

      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, height: '100vh' }}>
        <DashboardHeader showCreateButton onCreateAnnouncement={() => setIsModalOpen(true)} />

        <main style={{ flex: 1, overflowY: 'auto', padding: '36px 42px' }}>
          <div style={{ marginBottom: '32px' }}>
            <h1 style={{ fontSize: '38px', fontWeight: 800, color: '#0f172a', marginBottom: '8px', letterSpacing: '-0.03em' }}>
              Dashboard Overview
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6" style={{ marginBottom: '28px' }}>
            {statCards.map((card, i) => (
              <div key={i} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '18px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <p style={{ fontSize: '11px', fontWeight: 700, color: '#64748b', letterSpacing: '0.08em' }}>
                    {card.label}
                  </p>
                  <div
                    style={{
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', marginTop: '8px' }}>
                  ...
                  {card.change ? (
                    <span style={{ marginLeft: '10px', fontSize: '12px', fontWeight: 700, color: '#10b981' }}>
                      {card.change}
                    </span>
                  ) : null}
                </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="xl:col-span-2" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px' }}>
              <div
                style={{
                  padding: '16px 18px',
              ...
              >
                ...
                <button
                  type="button"
                  style={{ color: '#E53935', fontSize: '12px', fontWeight: 800, border: 'none', background: 'none', cursor: 'pointer' }}
                >
                  View All
                </button>
              </div>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      <th style={{ textAlign: 'left', padding: '12px 18px', color: '#94a3b8', fontSize: '11px' }}>Subject</th>
                      <th style={{ textAlign: 'left', padding: '12px 18px', color: '#94a3b8', fontSize: '11px' }}>Channel</th>
                      <th style={{ textAlign: 'left', padding: '12px 18px', color: '#94a3b8', fontSize: '11px' }}>Date</th>
                      <th style={{ textAlign: 'left', padding: '12px 18px', color: '#94a3b8', fontSize: '11px' }}>Reach</th>
                    </tr>
                  </thead>
                  <tbody>
                    {announcements.length > 0 ? (
                      announcements.map((ann, i) => (
                        <tr key={i}>
                          <td style={{ padding: '13px 18px', color: '#334155', fontWeight: 700, borderTop: '1px solid #f8fafc' }}>
                            {ann.title}
                          </td>
                          <td style={{ padding: '13px 18px', color: '#64748b', borderTop: '1px solid #f8fafc' }}>{ann.channel_name}</td>
                          <td style={{ padding: '13px 18px', color: '#64748b', borderTop: '1px solid #f8fafc' }}>
                          ...
                          <td style={{ padding: '13px 18px', color: '#64748b', fontWeight: 600, borderTop: '1px solid #f8fafc' }}>
                      ...
                      <tr>
                        <td colSpan="4" style={{ textAlign: 'center', padding: '42px', color: '#94a3b8', fontStyle: 'italic' }}>
                ...
                <div style={{ padding: '18px' }}>
                  <div style={{ marginBottom: '18px' }}>
                    ...
                    <div style={{ height: '6px', background: '#f1f5f9', borderRadius: '10px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: '14%', background: '#E53935', borderRadius: '10px' }} />
                    </div>
                  </div>
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', fontWeight: 800, marginBottom: '8px' }}>
                    ...
                    <div style={{ height: '6px', background: '#f1f5f9', borderRadius: '10px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: '24%', background: '#1E88E5', borderRadius: '10px' }} />
                    </div>
                  </div>
                </div>
              </div>
