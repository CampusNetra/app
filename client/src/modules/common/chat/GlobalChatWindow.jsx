import React, { useEffect, useMemo, useRef, useState } from "react";
import {
  ArrowLeft,
  Bell,
  Info,
  Megaphone,
  MoreVertical,
  Plus,
  Reply,
  Search,
  Send,
  Smile,
} from "lucide-react";
import { io } from "socket.io-client";
import api from "../../../api";
import "../../student/chat/chat-student.css";

const resolveSocketUrl = () => {
  const configuredSocket = import.meta.env.VITE_SOCKET_URL;
  if (configuredSocket && /^https?:\/\//.test(configuredSocket)) {
    return configuredSocket.replace(/\/$/, "");
  }

  const configuredApi = import.meta.env.VITE_API_URL;
  if (configuredApi && /^https?:\/\//.test(configuredApi)) {
    return configuredApi.replace(/\/api\/?$/, "");
  }

  // Use current origin so Vite proxy (/socket.io) works in local dev.
  return window.location.origin;
};

const GlobalChatWindow = ({
  channelId,
  initialChannel = null,
  initialUser = null,
  onBack,
  apiBasePath,
  tokenKeys,
  loadUser,
  canPostTopLevel,
  canReplyInThreads,
  enableOwnMessageActions = false,
}) => {
  const [channel, setChannel] = useState(initialChannel);
  const [user, setUser] = useState(initialUser);
  const [messages, setMessages] = useState([]);
  const [loading, setLoading] = useState(true);
  const [inputValue, setInputValue] = useState("");

  const [activeThread, setActiveThread] = useState(null);
  const [threadReplies, setThreadReplies] = useState([]);
  const [threadLoading, setThreadLoading] = useState(false);
  const [threadInput, setThreadInput] = useState("");

  const [isSearching, setIsSearching] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [showInfo, setShowInfo] = useState(false);
  const [channelDetails, setChannelDetails] = useState(null);
  const [showAllStudents, setShowAllStudents] = useState(false);
  const [isMuted, setIsMuted] = useState(false);
  const [memberSearchQuery, setMemberSearchQuery] = useState("");
  const [isSearchingMembers, setIsSearchingMembers] = useState(false);

  const [editingMessageId, setEditingMessageId] = useState(null);
  const [editValue, setEditValue] = useState("");
  const [confirmDeleteId, setConfirmDeleteId] = useState(null);
  const [showMoreMenuId, setShowMoreMenuId] = useState(null);

  const scrollRef = useRef(null);
  const threadScrollRef = useRef(null);
  const socketRef = useRef(null);
  const activeChannelRef = useRef(null);
  const activeThreadRef = useRef(null);
  const seenMessageIdsRef = useRef(new Set());
  const userRef = useRef(user);
  const lastMarkReadRef = useRef({ channelId: null, ts: 0 });

  const effectiveChannelId = useMemo(
    () => channelId || channel?.id || null,
    [channelId, channel?.id],
  );

  const canPostToChannel =
    typeof canPostTopLevel === "function" ? canPostTopLevel(channel) : true;
  const canReplyInChannel =
    typeof canReplyInThreads === "function" ? canReplyInThreads(channel) : true;

  useEffect(() => {
    setChannel(initialChannel || null);
  }, [initialChannel]);

  useEffect(() => {
    activeChannelRef.current = channel;
  }, [channel]);

  useEffect(() => {
    activeThreadRef.current = activeThread;
  }, [activeThread]);

  useEffect(() => {
    userRef.current = user;
  }, [user]);

  useEffect(() => {
    if (!loadUser || user) return;

    let mounted = true;
    const fetchUser = async () => {
      try {
        const loadedUser = await loadUser();
        if (mounted) setUser(loadedUser || null);
      } catch (err) {
        console.error("Failed to load chat user", err);
      }
    };

    fetchUser();
    return () => {
      mounted = false;
    };
  }, [loadUser, user]);

  useEffect(() => {
    return () => {
      if (socketRef.current) {
        socketRef.current.disconnect();
        socketRef.current = null;
      }
    };
  }, []);

  useEffect(() => {
    if (!effectiveChannelId) return;

    seenMessageIdsRef.current = new Set();
    setActiveThread(null);
    setThreadReplies([]);
    setThreadInput("");

    const loadForChannel = async () => {
      await Promise.all([
        fetchChannelDetails(effectiveChannelId),
        loadMessages(effectiveChannelId),
      ]);
      await markChannelAsRead(effectiveChannelId, true);
    };

    loadForChannel();

    const socket = ensureSocketConnected();
    if (socket) {
      socket.emit("chat:join-channel", { channelId: effectiveChannelId });
    }

    return () => {
      if (socket) {
        socket.emit("chat:leave-channel", { channelId: effectiveChannelId });
      }
    };
  }, [effectiveChannelId]);

  useEffect(() => {
    if (!effectiveChannelId) return;

    const markCurrentAsRead = () => {
      markChannelAsRead(effectiveChannelId);
    };

    const onVisibilityChange = () => {
      if (document.visibilityState === "visible") {
        markCurrentAsRead();
      }
    };

    window.addEventListener("focus", markCurrentAsRead);
    document.addEventListener("visibilitychange", onVisibilityChange);

    return () => {
      window.removeEventListener("focus", markCurrentAsRead);
      document.removeEventListener("visibilitychange", onVisibilityChange);
    };
  }, [effectiveChannelId, apiBasePath]);

  useEffect(() => {
    if (scrollRef.current && !activeThread) {
      scrollRef.current.scrollTop = scrollRef.current.scrollHeight;
    }
  }, [messages, activeThread]);

  useEffect(() => {
    if (threadScrollRef.current) {
      threadScrollRef.current.scrollTop = threadScrollRef.current.scrollHeight;
    }
  }, [threadReplies]);

  const pickToken = () => {
    for (const key of tokenKeys || []) {
      const value = localStorage.getItem(key);
      if (value) return value;
    }
    return null;
  };

  const addSeenId = (id) => {
    if (!id) return false;
    if (seenMessageIdsRef.current.has(id)) return false;
    seenMessageIdsRef.current.add(id);
    return true;
  };

  const ensureSocketConnected = () => {
    if (socketRef.current) return socketRef.current;

    const token = pickToken();
    if (!token) return null;

    const socket = io(resolveSocketUrl(), {
      auth: { token },
      transports: ["polling", "websocket"],
      reconnection: true,
      reconnectionAttempts: Infinity,
      reconnectionDelay: 1000,
      reconnectionDelayMax: 10000,
    });

    socket.on("connect", () => {
      const currentChannelId = activeChannelRef.current?.id || effectiveChannelId;
      if (currentChannelId) {
        socket.emit("chat:join-channel", { channelId: currentChannelId });
      }
    });

    socket.on("chat:new-message", (incomingMessage) => {
      if (!incomingMessage?.id) return;

      const currentChannelId = activeChannelRef.current?.id || effectiveChannelId;
      if (Number(incomingMessage.channel_id) !== Number(currentChannelId)) return;

      if (!addSeenId(incomingMessage.id)) return;

      if (incomingMessage.parent_id) {
        if (
          Number(activeThreadRef.current?.id) === Number(incomingMessage.parent_id)
        ) {
          setThreadReplies((prev) => {
            if (prev.some((msg) => msg.id === incomingMessage.id)) return prev;
            return [...prev, incomingMessage];
          });
        }

        setMessages((prev) =>
          prev.map((msg) => {
            if (Number(msg.id) !== Number(incomingMessage.parent_id)) return msg;
            return { ...msg, reply_count: Number(msg.reply_count || 0) + 1 };
          }),
        );
      } else {
        setMessages((prev) => {
          if (prev.some((msg) => msg.id === incomingMessage.id)) return prev;
          return [...prev, incomingMessage];
        });
      }

      if (Number(incomingMessage.sender_id) !== Number(userRef.current?.id)) {
        markChannelAsRead(currentChannelId);
      }
    });

    socket.on("user:presence", ({ userId, is_online, last_seen }) => {
      setChannelDetails((prev) => {
        if (!prev || !prev.members) return prev;
        return {
          ...prev,
          members: prev.members.map((member) =>
            Number(member.id) === Number(userId)
              ? { ...member, is_online, last_seen: last_seen || new Date() }
              : member,
          ),
        };
      });
    });

    socket.on("chat:message-edited", (editedMessage) => {
      const updateList = (list) =>
        list.map((m) => (m.id === editedMessage.id ? { ...m, ...editedMessage } : m));
      setMessages((prev) => updateList(prev));
      setThreadReplies((prev) => updateList(prev));
      setActiveThread((prev) =>
        prev && prev.id === editedMessage.id ? { ...prev, ...editedMessage } : prev,
      );
    });

    socket.on("chat:message-deleted", ({ id }) => {
      const removeList = (list) => list.filter((m) => m.id !== id);
      setMessages((prev) => removeList(prev));
      setThreadReplies((prev) => removeList(prev));
      if (activeThreadRef.current?.id === id) {
        setActiveThread(null);
        setThreadReplies([]);
      }
    });

    socketRef.current = socket;
    return socket;
  };

  const markChannelAsRead = async (id, force = false) => {
    if (!id) return;

    const now = Date.now();
    if (
      !force &&
      Number(lastMarkReadRef.current.channelId) === Number(id) &&
      now - lastMarkReadRef.current.ts < 1200
    ) {
      return;
    }

    lastMarkReadRef.current = { channelId: id, ts: now };

    try {
      await api.post(`${apiBasePath}/channels/${id}/read`);
    } catch (err) {
      console.error("Failed to mark channel as read", err);
    }
  };

  const fetchChannelDetails = async (id) => {
    try {
      const res = await api.get(`${apiBasePath}/channels/${id}`);
      setChannel(res.data || null);
      setChannelDetails(res.data || null);
    } catch (err) {
      console.error("Failed to fetch channel details", err);
    }
  };

  const loadMessages = async (id) => {
    try {
      setLoading(true);
      const response = await api.get(`${apiBasePath}/channels/${id}/messages`);
      const nextMessages = response?.data || [];
      setMessages(nextMessages);
      nextMessages.forEach((message) => addSeenId(message.id));
    } catch (err) {
      console.error("Failed to load messages", err);
    } finally {
      setLoading(false);
    }
  };

  const openThread = async (message) => {
    if (!message || message.parent_id) return;
    setActiveThread(message);
    setThreadLoading(true);
    try {
      const response = await api.get(`${apiBasePath}/messages/${message.id}/replies`);
      const replies = response?.data || [];
      setThreadReplies(replies);
      replies.forEach((reply) => addSeenId(reply.id));
    } catch (err) {
      console.error("Failed to load replies", err);
    } finally {
      setThreadLoading(false);
    }
  };

  const closeThread = () => {
    setActiveThread(null);
    setThreadReplies([]);
    setThreadInput("");
  };

  const handleSend = async () => {
    if (!effectiveChannelId || !inputValue.trim()) return;
    try {
      const response = await api.post(`${apiBasePath}/channels/${effectiveChannelId}/messages`, {
        content: inputValue,
        type: "text",
      });

      const newMessage = response?.data;
      setInputValue("");
      if (!newMessage || !addSeenId(newMessage.id)) return;

      setMessages((prev) => {
        if (prev.some((msg) => msg.id === newMessage.id)) return prev;
        return [...prev, newMessage];
      });
    } catch (err) {
      console.error("Send failed", err);
    }
  };

  const handleThreadSend = async () => {
    if (
      !effectiveChannelId ||
      !activeThread ||
      !threadInput.trim() ||
      !canReplyInChannel
    ) {
      return;
    }

    try {
      const response = await api.post(`${apiBasePath}/channels/${effectiveChannelId}/messages`, {
        content: threadInput,
        type: "text",
        parent_id: activeThread.id,
      });

      const newReply = response?.data;
      setThreadInput("");
      if (!newReply || !addSeenId(newReply.id)) return;

      setThreadReplies((prev) => {
        if (prev.some((reply) => reply.id === newReply.id)) return prev;
        return [...prev, newReply];
      });

      setMessages((prev) =>
        prev.map((msg) => {
          if (Number(msg.id) !== Number(activeThread.id)) return msg;
          return { ...msg, reply_count: Number(msg.reply_count || 0) + 1 };
        }),
      );
    } catch (err) {
      console.error("Reply failed", err);
    }
  };

  const handleDeleteMessage = async (messageId) => {
    try {
      await api.delete(`${apiBasePath}/messages/${messageId}`);
      setConfirmDeleteId(null);
    } catch (err) {
      console.error("Delete failed", err);
    }
  };

  const handleEditSubmit = async (messageId) => {
    if (!editValue.trim()) return;
    try {
      await api.put(`${apiBasePath}/messages/${messageId}`, {
        content: editValue,
      });
      setEditingMessageId(null);
      setEditValue("");
    } catch (err) {
      console.error("Edit failed", err);
    }
  };

  const getInitials = (name) => {
    if (!name) return "U";
    return name
      .split(" ")
      .map((n) => n[0])
      .join("")
      .toUpperCase()
      .slice(0, 2);
  };

  const formatTime = (dateStr) => {
    if (!dateStr) return "";
    const date = new Date(dateStr);
    return date.toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
      hour12: true,
    });
  };

  const filteredMessages = isSearching
    ? messages.filter((message) => {
        const content = (message.content || "").toLowerCase();
        const sender = (message.sender_name || "").toLowerCase();
        const query = searchQuery.toLowerCase();
        return content.includes(query) || sender.includes(query);
      })
    : messages;

  const renderMessageCard = (msg) => {
    const isAnnouncement =
      msg.type === "announcement" ||
      msg.sender_role === "faculty" ||
      msg.sender_role === "admin";
    const canReply = !msg.parent_id && (isAnnouncement || Number(msg.reply_count || 0) > 0);
    const isOwn = Number(msg.sender_id) === Number(user?.id);

    const isSystemAdmin =
      msg.sender_role === "dept_admin" ||
      msg.sender_role === "super_admin" ||
      msg.sender_role === "admin";
    const isFacultyRole = msg.sender_role === "faculty";
    const normalizedRole = isSystemAdmin
      ? "admin"
      : isFacultyRole
        ? "faculty"
        : "student";

    const roleColors = {
      admin: "bg-amber-100 text-amber-700 border-amber-200",
      faculty: "bg-indigo-100 text-indigo-700 border-indigo-200",
      student: "bg-emerald-100 text-emerald-700 border-emerald-200",
    };
    const roleDisplay = {
      admin: "Admin",
      faculty: "Faculty",
      student: "Student",
    };

    return (
      <div key={msg.id} className={`st-msg-wrap animate-fade ${isOwn ? "own" : ""}`}>
        {!isOwn && <div className="st-msg-avatar">{getInitials(msg.sender_name)}</div>}
        <div className="st-msg-main">
          {!isOwn && (
            <div className="st-msg-sender flex items-center gap-2 mb-1.5 px-1">
              <span className="st-sender-name font-black text-slate-800 text-[13px] tracking-tight">
                {msg.sender_name}
              </span>
              <span
                className={`px-2 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-wider ${roleColors[normalizedRole]}`}
              >
                {roleDisplay[normalizedRole]}
              </span>
            </div>
          )}

          <div
            className={`st-msg-bubble ${isAnnouncement ? "announcement" : ""} ${isOwn ? "own" : ""}`}
            onClick={() => canReply && openThread(msg)}
            style={{ cursor: canReply ? "pointer" : "default" }}
          >
            {isAnnouncement && (
              <div
                className="st-card-tag"
                style={{
                  color: isOwn ? "white" : "#ff6129",
                  fontSize: "10px",
                  fontWeight: 900,
                  marginBottom: "8px",
                  display: "flex",
                  alignItems: "center",
                  gap: "4px",
                }}
              >
                <Megaphone size={12} fill="currentColor" /> IMPORTANT SIGNAL
              </div>
            )}

            {editingMessageId === msg.id ? (
              <div className="flex flex-col gap-2">
                <textarea
                  className="w-full bg-white/10 text-white border border-white/20 rounded-lg p-2 text-sm outline-none"
                  value={editValue}
                  onChange={(e) => setEditValue(e.target.value)}
                  autoFocus
                />
                <div className="flex gap-2 justify-end">
                  <button
                    onClick={() => setEditingMessageId(null)}
                    className="text-[10px] font-black text-white/70"
                  >
                    CANCEL
                  </button>
                  <button
                    onClick={() => handleEditSubmit(msg.id)}
                    className="text-[10px] font-black text-white bg-white/20 px-2 py-1 rounded"
                  >
                    SAVE
                  </button>
                </div>
              </div>
            ) : (
              <div className="st-msg-text">
                {msg.content}
                {msg.is_edited ? (
                  <span className="text-[10px] opacity-40 ml-2">(edited)</span>
                ) : null}
              </div>
            )}

            <div className="st-msg-footer">
              <span className="st-msg-time-internal">{formatTime(msg.created_at)}</span>
            </div>
          </div>

          <div className="st-card-actions flex flex-row items-center gap-3">
            <button className="st-action-btn flex items-center gap-1" onClick={() => openThread(msg)}>
              <Reply size={14} /> REPLY
            </button>

            {enableOwnMessageActions && isOwn ? (
              <div className="relative">
                <button
                  className="st-action-btn p-1 -m-1"
                  onClick={(e) => {
                    e.stopPropagation();
                    setShowMoreMenuId(showMoreMenuId === msg.id ? null : msg.id);
                  }}
                >
                  <MoreVertical size={16} />
                </button>

                {showMoreMenuId === msg.id && (
                  <div className="absolute right-0 top-full mt-1 bg-white shadow-2xl rounded-2xl border border-slate-100 py-2 z-[1000] animate-fade min-w-[140px]">
                    <button
                      className="w-full px-5 py-3 text-left text-sm font-black text-slate-700 hover:bg-slate-50"
                      onClick={() => {
                        setEditingMessageId(msg.id);
                        setEditValue(msg.content);
                        setShowMoreMenuId(null);
                      }}
                    >
                      Edit Signal
                    </button>
                    <button
                      className="w-full px-5 py-3 text-left text-sm font-black text-red-500 hover:bg-red-50"
                      onClick={() => {
                        setConfirmDeleteId(msg.id);
                        setShowMoreMenuId(null);
                      }}
                    >
                      Delete
                    </button>
                  </div>
                )}
              </div>
            ) : null}

            {Number(msg.reply_count || 0) > 0 ? (
              <button
                className="st-action-btn"
                style={{ color: "#94a3b8" }}
                onClick={() => openThread(msg)}
              >
                <Smile size={14} /> {msg.reply_count}
              </button>
            ) : null}
          </div>
        </div>
      </div>
    );
  };

  if (loading && !channel) {
    return (
      <div className="st-shell items-center justify-center flex">
        <div className="w-10 h-10 border-4 border-slate-50 border-t-orange-600 rounded-full animate-spin"></div>
      </div>
    );
  }

  return (
    <div className="st-chat-window animate-fade">
      {!activeThread && !showInfo ? (
        <div className="st-chat-shell">
          <header className={`st-win-header-v2 ${isSearching ? "searching" : ""}`}>
            {!isSearching ? (
              <>
                <button onClick={onBack} className="st-icon-btn">
                  <ArrowLeft size={24} />
                </button>
                <div className="st-win-info">
                  <h1 className="font-black tracking-tight">{channel?.name || "Group Chat"}</h1>
                  <p className="font-bold text-slate-400">
                    {channelDetails?.members?.length || channel?.member_count || 0} participants
                  </p>
                </div>
                <div className="flex gap-4">
                  <button className="st-icon-btn" onClick={() => setIsSearching(true)}>
                    <Search size={22} />
                  </button>
                  <button className="st-icon-btn" onClick={() => setShowInfo(true)}>
                    <Info size={22} />
                  </button>
                </div>
              </>
            ) : (
              <div className="st-header-search-box">
                <button
                  onClick={() => {
                    setIsSearching(false);
                    setSearchQuery("");
                  }}
                  className="st-icon-btn"
                >
                  <ArrowLeft size={22} />
                </button>
                <input
                  autoFocus
                  type="text"
                  placeholder="Search in chat..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
            )}
          </header>

          <div className="st-chat-messages custom-scrollbar" ref={scrollRef}>
            {loading ? (
              <div className="p-20 text-center">
                <div className="w-8 h-8 border-4 border-slate-100 border-t-orange-500 rounded-full animate-spin mx-auto"></div>
              </div>
            ) : filteredMessages.length > 0 ? (
              <>
                {!isSearching ? (
                  <div className="st-date-separator">
                    <div className="st-date-pill">TODAY</div>
                  </div>
                ) : null}
                {filteredMessages.map(renderMessageCard)}
              </>
            ) : (
              <div className="p-20 text-center text-slate-400">
                {isSearching ? "No results found for your search." : "No signals found here yet."}
              </div>
            )}
          </div>

          {canPostToChannel ? (
            <div className="st-chat-input-bar">
              <button className="st-icon-btn text-slate-400">
                <Plus size={26} />
              </button>
              <div className="st-input-container">
                <input
                  type="text"
                  placeholder="Type a message..."
                  value={inputValue}
                  onChange={(e) => setInputValue(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && handleSend()}
                />
                <Smile size={22} className="text-slate-400" />
              </div>
              <button className="st-send-btn shadow-lg" onClick={handleSend}>
                <Send size={24} fill="currentColor" />
              </button>
            </div>
          ) : null}
        </div>
      ) : showInfo ? (
        <div className="st-info-page animate-fade flex flex-col h-full bg-white">
          <header className="st-win-header-v2 border-none">
            <button onClick={() => setShowInfo(false)} className="st-icon-btn">
              <ArrowLeft size={24} />
            </button>
            <div className="st-win-info">
              <h1>Group Info</h1>
            </div>
            <button className="st-icon-btn">
              <MoreVertical size={24} />
            </button>
          </header>

          <div className="flex-1 overflow-y-auto custom-scrollbar">
            <div className="st-info-hero">
              <div className="st-hero-avatar-wrap relative">
                <div className="st-hero-avatar">{getInitials(channel?.name)}</div>
                <div className="absolute bottom-1 right-1 w-8 h-8 bg-orange-500 border-4 border-white rounded-full flex items-center justify-center text-white">
                  <Smile size={16} fill="currentColor" />
                </div>
              </div>

              <h2 className="st-hero-name">{channel?.name}</h2>
              <p className="st-hero-stats">
                {channelDetails?.members?.length || 0} Members · {channelDetails?.members?.filter((m) =>
                  m.role === "admin" ||
                  m.role === "dept_admin" ||
                  m.role === "super_admin" ||
                  m.channel_role === "admin" ||
                  m.channel_role === "owner",
                ).length || 0} Admins
              </p>
            </div>

            <div className="st-info-content px-5 pb-24">
              <div className="st-info-section">
                <div className="st-info-section-header">
                  <span>ADMINS</span>
                </div>
                <div className="st-info-list">
                  {channelDetails?.members
                    ?.filter(
                      (member) =>
                        member.role === "admin" ||
                        member.role === "dept_admin" ||
                        member.role === "super_admin" ||
                        member.channel_role === "owner",
                    )
                    .map((member) => (
                      <div key={member.id} className="st-info-member-item">
                        <div className="st-member-avatar relative bg-orange-500 text-white text-sm">
                          {getInitials(member.name)}
                          {member.is_online ? (
                            <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></span>
                          ) : null}
                        </div>
                        <div className="st-member-info">
                          <div className="st-member-name flex items-center gap-2">
                            {member.name}
                            <span className="px-1.5 py-0.5 bg-orange-100 text-orange-600 rounded text-[9px] font-bold uppercase tracking-tight">
                              Admin
                            </span>
                          </div>
                        </div>
                      </div>
                    ))}
                </div>
              </div>

              <div className="st-info-section mt-6">
                <div className="st-info-section-header">
                  <span>FACULTY</span>
                </div>
                <div className="st-info-list">
                  {channelDetails?.members
                    ?.filter((member) => member.role === "faculty")
                    .map((member) => (
                      <div key={member.id} className="st-info-member-item">
                        <div className="st-member-avatar relative bg-slate-100 text-slate-500 text-sm">
                          {getInitials(member.name)}
                          {member.is_online ? (
                            <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></span>
                          ) : null}
                        </div>
                        <div className="st-member-info">
                          <div className="st-member-name flex items-center gap-2">{member.name}</div>
                        </div>
                      </div>
                    ))}
                </div>
              </div>

              <div className="st-info-section mt-8">
                <div className="st-info-section-header">
                  <span>STUDENTS</span>
                  <button
                    className="text-orange-500 font-bold ml-auto text-[13px]"
                    onClick={() => setIsSearchingMembers(!isSearchingMembers)}
                  >
                    {isSearchingMembers ? "Cancel" : "Search"}
                  </button>
                </div>

                {isSearchingMembers ? (
                  <div className="mt-4 mb-2 animate-fade">
                    <input
                      type="text"
                      placeholder="Search for a student..."
                      className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3 text-sm outline-none"
                      value={memberSearchQuery}
                      onChange={(e) => setMemberSearchQuery(e.target.value)}
                    />
                  </div>
                ) : null}

                <div className="st-info-list">
                  {channelDetails?.members
                    ?.filter((member) => member.role === "student")
                    .filter(
                      (member) =>
                        !memberSearchQuery ||
                        member.name
                          ?.toLowerCase()
                          .includes(memberSearchQuery.toLowerCase()),
                    )
                    .slice(0, showAllStudents || isSearchingMembers ? 999 : 5)
                    .map((member) => (
                      <div key={member.id} className="st-info-member-item">
                        <div className="st-member-avatar relative bg-slate-100 text-slate-400 text-sm">
                          {getInitials(member.name)}
                          {member.is_online ? (
                            <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></span>
                          ) : null}
                        </div>
                        <div className="st-member-name flex-1 flex flex-col">
                          <span>{member.name}</span>
                          {member.is_online ? (
                            <span className="text-[10px] text-green-500 font-bold">Active Now</span>
                          ) : member.last_seen ? (
                            <span className="text-[10px] text-slate-400 font-medium">
                              Last seen: {formatTime(member.last_seen)}
                            </span>
                          ) : (
                            <span className="text-[10px] text-slate-400 font-medium">Offline</span>
                          )}
                        </div>
                      </div>
                    ))}

                  {channelDetails?.members?.filter((member) => member.role === "student").length > 5 &&
                  !showAllStudents &&
                  !isSearchingMembers ? (
                    <button
                      onClick={() => setShowAllStudents(true)}
                      className="w-full py-4 text-slate-400 text-sm font-bold flex items-center justify-center gap-2"
                    >
                      <Plus size={16} /> Show more members
                    </button>
                  ) : null}
                </div>
              </div>

              <div className="st-info-settings mt-8 bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                <div className="flex items-center gap-4 p-4 border-b border-slate-50">
                  <div className="w-10 h-10 rounded-xl bg-slate-50 text-slate-500 flex items-center justify-center">
                    <Bell size={20} />
                  </div>
                  <div className="flex-1">
                    <div className="font-bold text-slate-700">Mute Notifications</div>
                    <div className="text-[10px] text-slate-400 font-medium">
                      Turn off all message alerts
                    </div>
                  </div>
                  <div
                    className={`w-10 h-6 rounded-full p-1 cursor-pointer ${isMuted ? "bg-orange-500" : "bg-slate-200"}`}
                    onClick={() => setIsMuted(!isMuted)}
                  >
                    <div
                      className={`w-4 h-4 bg-white rounded-full shadow-sm ${isMuted ? "translate-x-4" : "translate-x-0"}`}
                    ></div>
                  </div>
                </div>
                <div className="flex items-center gap-4 p-4">
                  <div className="w-10 h-10 rounded-xl bg-slate-50 text-slate-500 flex items-center justify-center">
                    <Info size={20} />
                  </div>
                  <span className="flex-1 font-bold text-slate-700">Report Group</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      ) : (
        <div className="st-thread-page animate-fade">
          <header className="st-win-header-v2">
            <button onClick={closeThread} className="st-icon-btn">
              <ArrowLeft size={24} />
            </button>
            <div className="st-win-info">
              <h1>Thread Discussion</h1>
              <p>
                {threadReplies.length} {threadReplies.length === 1 ? "Reply" : "Replies"}
              </p>
            </div>
            <button className="st-icon-btn">
              <MoreVertical size={24} />
            </button>
          </header>

          <div className="st-thread-rail-wrap custom-scrollbar" ref={threadScrollRef}>
            <div className="st-thread-rail"></div>

            <div className="pt-6">
              <div className="st-original-pill">Original Announcement</div>
              {activeThread ? (
                <div className="st-thread-item is-root">
                  <div className="st-thread-avatar flex items-center justify-center bg-orange-500 text-white font-bold text-[12px]">
                    {getInitials(activeThread.sender_name)}
                  </div>
                  <div className="st-thread-card">
                    <div className="st-thread-meta flex items-center justify-between gap-4 mb-1">
                      <span className="font-bold text-[14px] text-slate-800 flex items-center gap-2">
                        {activeThread.sender_name}
                        {activeThread.sender_role === "faculty" ? (
                          <span className="st-thread-badge faculty">Faculty</span>
                        ) : null}
                      </span>
                      <span className="text-[10px] text-slate-400 font-medium uppercase tracking-tight">
                        {formatTime(activeThread.created_at)}
                      </span>
                    </div>
                    <div className="text-[14px] text-slate-700 leading-relaxed">{activeThread.content}</div>
                  </div>
                </div>
              ) : null}
            </div>

            <div
              className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 mb-4"
              style={{ marginLeft: "60px" }}
            >
              {threadReplies.length} {threadReplies.length === 1 ? "Reply" : "Replies"}
            </div>

            <div className="pb-32">
              {threadLoading ? (
                <div className="p-12 text-center">
                  <div className="w-6 h-6 border-2 border-slate-100 border-t-orange-500 rounded-full animate-spin mx-auto"></div>
                </div>
              ) : (
                threadReplies.map((reply) => (
                  <div key={reply.id} className="st-thread-item animate-fade">
                    <div className="st-thread-avatar flex items-center justify-center bg-slate-200 text-slate-600 font-bold text-[10px]">
                      {getInitials(reply.sender_name)}
                    </div>
                    <div className={`st-thread-card ${reply.sender_role === "faculty" ? "faculty-reply" : ""}`}>
                      <div className="st-thread-meta flex items-center justify-between gap-4 mb-1">
                        <span className="font-bold text-[13px] text-slate-800 flex items-center gap-2">
                          {reply.sender_name}
                          {reply.sender_role === "faculty" ? (
                            <span className="st-thread-badge faculty">Faculty</span>
                          ) : null}
                        </span>
                        <span className="text-[10px] text-slate-400 font-medium uppercase tracking-tight">
                          {formatTime(reply.created_at)}
                        </span>
                      </div>
                      <div className="text-[14px] text-slate-700">{reply.content}</div>
                    </div>
                  </div>
                ))
              )}
            </div>
          </div>

          {canReplyInChannel ? (
            <div className="st-chat-input-bar">
              <button className="st-icon-btn text-slate-400">
                <Plus size={26} />
              </button>
              <div className="st-input-container">
                <input
                  type="text"
                  placeholder="Reply to thread..."
                  value={threadInput}
                  onChange={(e) => setThreadInput(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && handleThreadSend()}
                />
                <Smile size={22} className="text-slate-400" />
              </div>
              <button className="st-send-btn shadow-lg" onClick={handleThreadSend} disabled={threadLoading}>
                <Send size={24} fill="currentColor" />
              </button>
            </div>
          ) : null}
        </div>
      )}

      {confirmDeleteId ? (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[999] flex items-center justify-center p-6"
          onClick={() => setConfirmDeleteId(null)}
        >
          <div
            className="bg-white rounded-3xl w-full max-w-sm overflow-hidden animate-fade"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-8 text-center">
              <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-6">
                <MoreVertical size={32} className="rotate-90" />
              </div>
              <h3 className="text-xl font-bold text-slate-800 mb-2">Delete Message?</h3>
              <p className="text-slate-500 text-sm leading-relaxed">
                Are you sure you want to delete this message? This action is permanent and cannot be undone.
              </p>
            </div>
            <div className="flex border-t border-slate-100">
              <button
                onClick={() => setConfirmDeleteId(null)}
                className="flex-1 py-5 text-sm font-bold text-slate-400 border-r border-slate-100 hover:bg-slate-50"
              >
                CANCEL
              </button>
              <button
                onClick={() => handleDeleteMessage(confirmDeleteId)}
                className="flex-1 py-5 text-sm font-bold text-red-500 hover:bg-red-50"
              >
                DELETE
              </button>
            </div>
          </div>
        </div>
      ) : null}
    </div>
  );
};

export default GlobalChatWindow;